"""
Q3/new_algorithm.py
CABS: Community-Aware Beam Search for Network Dismantling
基于社区桥梁启发式与双连通核指标 Q_bc 的两阶段拆解算法。

两阶段策略
----------
Phase A — 贪心预热（Greedy Warm-up）
    在图仍然庞大的阶段，用 Leiden 社区外部度贪心逐节点删除。
    **关键：候选节点仅从当前 LCC 内选取**，避免浪费在碎片节点上。

Phase B — 集束精细搜索（Beam Search Refinement）
    在缩小后的图上执行 Beam Search：
    - **候选节点仅从 LCC 内部选取**（修复旧版 LCC 停滞不降的 bug）
    - 候选评估多线程并行化
    - 支持批量删除（每步删 batch_size 个节点）以减少迭代数
    - Leiden 保证社区连通，k_ext 语义精确

修复说明
--------
旧版的核心 bug：预热后图有 ~17000 节点但 LCC 仅 ~450 节点。
Leiden + ext_deg 在全图上运行，选出的高外部度节点大多在小碎片里，
删掉它们对 LCC 完全没有影响，导致 LCC/N_0 停在 0.0248 不再下降。
修复方案：所有候选筛选都限定在 LCC 内部节点中进行。
"""
...
def get_lcc_info(G: ig.Graph) -> tuple[float, int, list[int]]:
    """
    ...
    lcc_members = cc[lcc_idx]  # igraph 内部索引列表
...
def _evaluate_candidate(args) -> dict:
    """线程工作函数：评估删除单个候选节点后的 Q_bc。"""
    G_b, v_idx, nid, R_b, N0, delta_f, alpha, beta, b_idx = args
...
def greedy_warmup(
    ...
    **候选节点仅从 LCC 内部选取**。
    ...
    策略：
    - 每隔 leiden_refresh 步：在 LCC 子图上做 Leiden，按 ext_deg 排序，
      预生成接下来 leiden_refresh 步的删除名单（按 node_id 存储）。
    - 中间步骤：按名单顺序删除，跳过已不在图中或不在 LCC 中的节点。
    - 如果名单耗尽则立刻刷新。
    ...
    print(
        ...
        f"LCC/N_0={lcc:.4f}, R={cumR:.6f}, 耗时={elapsed:.1f}s"
    )
...
# =====================================================================
# Phase B: Beam Search（候选限定在 LCC 内）
# =====================================================================
    ...
    while True:
        ...
        for b_idx, (G_b, seq_b, R_b, P_b) in enumerate(beam):
            ...
            for orig_idx, nid in candidates:
                eval_tasks.append((
                    G_b, orig_idx, nid,
                    R_b, N0, delta_f, alpha, beta, b_idx,
                ))
        ...
        for sel in selected:
            ...
            # 追加 (batch_size - 1) 个节点：从 LCC 中贪心选择
            for _ in range(batch_size - 1):
                ...
                qbc_g = compute_performance(G_new, N0, alpha, beta)
                new_R += qbc_g * delta_f
...
def cabs_attack(
    ...
    print(
        f"\n  [{city_name}] 全部完成! "
        f"序列长度={len(full_seq)}, R_bc={full_R:.6f}"
    )
...
def main():
    ...
    strategy_name = f"CABS_W{W}_K{K}_B{batch_size}"
    ...
    for filename in sorted(os.listdir(json_dir)):
        ...
        print(f"\n{'='*64}")
        print(f"  城市: {city} — CABS 社区桥梁集束搜索拆解")
        print(f"{'='*64}")

        t0 = time.perf_counter()

        # ---- 两阶段攻击 ----
        seq, R_bc = cabs_attack(
            json_path,
            W=W, K=K, batch_size=batch_size,
            alpha=alpha, beta=beta, theta=theta,
        ...
        summary_rows.append({
            ...
            "R_bc_beam": R_bc,
            "q_critical_05": q_critical,
            "Nodes_Removed": len(seq),
            "Time_seconds": elapsed,
        })

    # ── 保存 ──
    pq_csv = os.path.join(out_dir, "Q3_CABS_BeamSearch_Data_Pq.csv")
    pd.DataFrame(all_data_pq).to_csv(pq_csv, index=False, encoding="utf-8-sig")

    qbc_csv = os.path.join(out_dir, "Q3_CABS_BeamSearch_Data_Qbc.csv")
    pd.DataFrame(all_data_qbc).to_csv(qbc_csv, index=False, encoding="utf-8-sig")

    summary_csv = os.path.join(out_dir, "Q3_CABS_BeamSearch_Summary.csv")
    df_summary = pd.DataFrame(summary_rows)
    df_summary.to_csv(summary_csv, index=False, encoding="utf-8-sig")

    print(f"\n{'='*64}")
    print("  全部城市 CABS 拆解完毕！")
    print(f"{'='*64}")
    print("\n各城市 R_bc 排名（升序 = 攻击越有效）：")
    print(df_summary.sort_values("R_bc_trapz").to_string(index=False))
