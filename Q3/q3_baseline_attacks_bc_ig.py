...
def get_attack_sequence_betweenness(G, city_name):
    """策略 2：介数中心性攻击序列 (从 Q1 导出的 CSV 读取)"""
...
def get_attack_sequence_closeness(G, city_name):
    """策略 3：接近度中心性攻击序列 (从 Q1 导出的 CSV 读取)"""
...
def simulate_targeted_attack(G, id2idx, sorted_nodes, step_size=0.01, city_name="", attack_type=""):
    ...
    for i in range(1, len(q_points)):
        ...
        # 触发独立评价函数
        qbc_val = eval_teammate_q_bc(G_temp, N0)
        Q_bc_vals.append(qbc_val)

        # 记录 Q_bc 首次跌破 1% 的临界批次
        if not reached_critical and qbc_val <= 0.01:
            reached_critical = True
            print(f"      [网络崩溃] q={q:.2f} 时结构健壮度(Q_bc)降至 {qbc_val:.4f} (<= 1%)！")
    ...
    for filename in os.listdir(json_dir):
        if filename.endswith("_Network.json"):
            ...
            for strategy_name, sequence_func in strategy_map.items():
                ...
                print("    - 执行滑动窗口接力处决 (仅跟踪 teammate Q_bc)...")
                ...
                for q, qbc in zip(q_pts, qbc_arr):
                    all_data.append({"City": city, "Strategy": strategy_name, "q": q, "Q_bc_Teammate": qbc})

    df_all = pd.DataFrame(all_data)
    csv_out = os.path.join(out_dir, "Q3_Baseline_Attacks_Data_IG_Teammate.csv")
    df_all.to_csv(csv_out, index=False, encoding="utf-8-sig")
    print(f"\n队友自定义指标 Q_bc(f) 防御测试完毕，数据存放至: {csv_out}")
